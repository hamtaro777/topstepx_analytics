...
from services.analytics import AnalyticsService, format_duration
from services.data_collector import DataCollector
from dashboard.components.metrics import render_kpi_row, render_trade_stats
from dashboard.components.charts import (
    create_equity_curve, create_daily_pnl_bar,
    create_direction_pie, create_duration_chart,
    create_win_rate_by_duration
)
...
<style>
    ...
    .metric-card { background: #1E1E1E; padding: 15px; border-radius: 8px; }
</style>
...
def render_overview_page(trades: list, daily_stats: pd.DataFrame):
    ...
    with col3:
        fig = create_direction_pie(metrics['long_pct'], metrics['short_pct'])
        st.plotly_chart(fig, use_container_width=True, key="direction_pie")
